...
class AbstractRouter(TimeActor):
    def __init__(self):
        super().__init__()
        self.overlord = None
        self.addr = None
    ...
    def initialize(self, message, sender):
        ...
        if isinstance(message, AbstractRouterInitMsg):
...
class LinkStateRouter(Router, LinkStateHolder):
    ...
    def _currentStateData(self, pkg):
        opn_enc = np.zeros(len(self.network))
        for (m, count) in self.outgoing_pkgs_num:
            opn_enc[m] = np.log(count + 1)
        return mk_current_neural_state(self.network_graph, self.current_time, pkg, self.addr, opn_enc)
